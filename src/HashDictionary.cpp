 ...
 *
 * Calculate and return the probe offset
 * using a quadratic probe function.
 *
 * @param key The key being hashed/probed. unused for a quadratic probe sequence.
 *
 ...
 */
template<class Key, class Value>
int HashDictionary<Key, Value>::probe(const Key& key, int probeIndex) const
{
  int c1 = 1;
  int c2 = 2;
  int c3 = 2;
  int result = c1 * (probeIndex * probeIndex) + c2 * probeIndex + c3;
  return result;
}
/**
 * @brief calculate hash index for key.
 *
 * Calculate and return the initial hash key index using a mid square
 * hashing function.
 *
 * @param key The key being hashed/probed. unused for a quadratic probe sequence.
 *
 * @returns int returns the calculated initial hash key index.
 */
template<class Key, class Value>
int HashDictionary<Key, Value>::hash(const Key& key) const
{
  int key2 = key * key;
  key2 = key2 & 0x00FFFFFF;
  key2 = key2 >> 8;
  int result = key2 % allocationSize;

  return result;
}

/**
 * @brief Search for an available spot.
 *
 * This function searches for the first empty / missing slot
 * within the table.
 *
 * @param key The key being hashed/probed. unused for a quadratic probe sequence.
 *
 * @returns int returns the probe index that is empty/missing.
 */
template<class Key, class Value>
int HashDictionary<Key, Value>::probeForAvailableSlot(const Key& key) const
{
  int probeSlot;
  int index = 0;
  while (index <= allocationSize)
  {
    probeSlot = (hash(key) + probe(key, index)) % allocationSize;
    if (hashTable[probeSlot].isMissing() or hashTable[probeSlot].isEmpty())
    {
      return probeSlot;
    }
    index++;
 ...
 * @brief calculate hash index for key.
 *
 * This function searches the table for an empty slot
 * or until the key finds the same value within the table.
 *
 * @param key The key being hashed/probed. unused for a quadratic probe sequence.
 *
 * @returns int returns the probe index of the position matching the key or an empty slot.
 */
template<class Key, class Value>
int HashDictionary<Key, Value>::probeForKeySlot(const Key& key) const
{
  ...
  {
    probeSlot = (hash(key) + probe(key, index)) % allocationSize;
    if (hashTable[probeSlot].isEmpty() or hashTable[probeSlot].getKey() == key)
    {
      return probeSlot;
    }
    index++;
 ...
 * @brief Inserts a key.
 *
 * This function inserts a key if it isn't a duplicate.
 * If the key already exists within the table, it will throw an
 * exception.
 *
 * @param key The key being hashed/probed. unused for a quadratic probe sequence.
 *
 * @param value The value being insterted into the table.
 */
template<class Key, class Value>
void HashDictionary<Key, Value>::insert(const Key& key, const Value& value)
{
  if (hashTable[probeForKeySlot(key)].getKey() == key)
  {
    throw DictionaryDuplicateKeyInsertionException("This is a dublicate key.");
  }
  growHashDictionaryIfNeeded();
  hashTable[probeForAvailableSlot(key) % allocationSize] = KeyValuePair<Key, Value>(key, value);
  HashDictionary::size++;
}
 ...
 * @brief Finds a key.
 *
 * This function finds a key in the table and returns its value.
 * If the key is empty within the table, it will throw an
 * exception.
 *
 * @param key The key being hashed/probed. unused for a quadratic probe sequence.
 *
 * @return Value The value within the key that was searched for.
 */
template<class Key, class Value>
Value HashDictionary<Key, Value>::find(const Key& key) const
{
  ...
  {
    throw DictionaryKeyNotFoundException("Key is not found.");
  }
 ...
 * @brief Removes a key.
 *
 * This function removes a key in the table and returns its value.
 * If the key is empty within the table, it will throw an
 * exception.
 *
 * @param key The key being hashed/probed. unused for a quadratic probe sequence.
 *
 * @return Value The value within the key that was removed.
 */
template<class Key, class Value>
Value HashDictionary<Key, Value>::remove(const Key& key)
{
  ...
  Value slot = hashTable[probeForKeySlot(key)].getValue();
